...
Authenticated smoke test 應在對應 Compose override 啟用後執行，並確認 extraction、CDN raster preview、generated fallback preview 與 download 均成功。缺少 poster 的項目應只在首次載入生成一次，後續請求命中 bounded cache。下載與 generated preview 階段不得攜帶 Cookie；若 CDN 需要登入 Cookie，系統必須 fail closed 為 `upstream_media_invalid`，不可改用平台 session 轉送。Cookie 輪替時先在平台撤銷舊 session，替換 host file，重新建立 container（避免 bind mount inode 保留舊內容），再以新的 account-visible URL 驗證。
